...
public sealed class TenantMembershipGrain : Grain, ITenantMembershipGrain
{
    private readonly IPersistentState<TenantMembershipState> _state;

    public TenantMembershipGrain(
        [PersistentState("membership", "Default")] IPersistentState<TenantMembershipState> state)
    {
        _state = state;
    }

    public Task<TenantMembershipState?> GetStateAsync()
    {
        if (string.IsNullOrEmpty(_state.State.UserId))
            return Task.FromResult<TenantMembershipState?>(null);

        return Task.FromResult<TenantMembershipState?>(_state.State);
    }

    public async Task<CreateMembershipResult> CreateAsync(CreateMembershipRequest request)
    {
        if (!string.IsNullOrEmpty(_state.State.UserId))
        {
        ...
        _state.State = new TenantMembershipState
        {
        ...
        return new CreateMembershipResult { Success = true };
    }

    public async Task UpdateAsync(UpdateMembershipRequest request)
    {
        if (request.Username != null)
            _state.State.Username = request.Username;

        _state.State.LastAccessAt = DateTime.UtcNow;
        await _state.WriteStateAsync();
    }
